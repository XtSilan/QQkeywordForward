...
“系统设置”提供跨群重复消息冷却。默认同一“关键词 + 完整消息正文”的前两次命中正常提醒，第 3 次开始进入 10 分钟冷却；正文不同的消息即使命中同一关键词也不会互相影响。SMTP 可通过 `.env.prod` 配置，也可以在“系统设置”保存。提醒包含群名、群号、发送者昵称与 QQ、命中关键词、时间和消息正文，失败会进入后台重试。
