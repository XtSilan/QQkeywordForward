...
“关键词”页面输入普通文本即可，系统内部使用安全的字面量正则匹配。每个群可以独立设置启用状态和冷却时间。

“通知”页面添加 QQ 好友或邮箱，并按群打开 QQ/邮件开关。SMTP 可通过 `.env.prod` 配置，也可以在“系统设置”保存。邮件包含群名、群号、发送者、命中关键词、时间和消息正文，失败会进入后台重试。
